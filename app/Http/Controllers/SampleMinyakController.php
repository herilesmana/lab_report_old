...
class SampleMinyakController extends Controller
{
    ...
    public function per_status($status, $status_reject = '')
    {
        $sample = DB::table('t_sample_minyak')
                  ->join('t_pv', 't_sample_minyak.id', '=', 't_pv.sample_id')
                  ->join('t_ffa', 't_sample_minyak.id', '=', 't_ffa.sample_id')
                  ->select('t_sample_minyak.*', 't_pv.tangki', 't_pv.id as pv_id', 't_ffa.id as ffa_id', 't_pv.volume_titrasi as volume_titrasi_pv', 't_pv.bobot_sample as bobot_sample_pv', 't_pv.normalitas as normalitas_pv', 't_pv.nilai as nilai_pv', 't_ffa.volume_titrasi as volume_titrasi_ffa', 't_ffa.bobot_sample as bobot_sample_ffa', 't_ffa.normalitas as normalitas_ffa', 't_ffa.nilai as nilai_ffa')
    ...
    public function store_sample(Request $request)
    {
        for ($i=0; $i <= $request['row']; $i++) {
            if ($request['nilai_pv_'.$i] != '' && $request['nilai_ffa_'.$i] != '' && $request['nilai_pv_'.$i] != 0 && $request['nilai_ffa_'.$i] != 0) {
                // Untuk kebutuhan lain
